...
export class Deck {
    ...
    constructor(blacks: CompactBlackCard[], whites: string[], seed: string, totalPlayers: number, playerIndex: number) {
...
export class Game {
    ...
    submitCards(indices: number[]) {
        ...
        const newWhiteCards = newCards.filter(c => c !== null).map(c => c as string)
